...
import { Clock, MapPin, CloudSun, Sun, CloudRain, Sparkles, ArrowRight } from "lucide-react"
...
function SkeletonCard() {
  return (
    <div className="flex min-h-[200px] w-full min-w-[220px] flex-1 shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
...
function TripCard({ trip }: { trip: OutdoorTodayTrip }) {
  ...
  const priceDisplay = nextSlot?.priceDisplay ?? (trip.price != null ? `€${trip.price}` : null)

  return (
    <Link
      href={`/trip/${trip.id}`}
      className="group relative flex min-h-[200px] w-full min-w-[220px] flex-1 shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md"
    >
      {/* Image */}
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      {isLoading ? (
        ...
        <div className="mt-4 flex flex-1 items-center justify-center rounded-2xl border border-dashed border-border bg-secondary/30 p-8 text-center">
          <div>
        ...
        <div className="mt-4 flex flex-1 gap-4 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
