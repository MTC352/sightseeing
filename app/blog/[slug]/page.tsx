...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) {
    return { title: "Post Not Found | sightseeing.lu" }
  }
...
export default async function BlogPostPage({ params }: Props) {
  ...
  // Simple markdown-ish rendering for the body
  const renderBody = (body: string) => {
    // Split by paragraphs and render with basic markdown support
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative">
        ...
        <div className="relative mx-auto max-w-3xl px-4 pb-10 pt-32 lg:pt-44 lg:pb-14">
          ...
          <h1 className="mt-3 text-2xl font-bold text-white lg:text-4xl leading-tight">
            {post.title}
          </h1>
      ...
      <article className="mx-auto max-w-3xl px-4 py-10 lg:py-14">
        <p className="mb-6 text-lg leading-relaxed text-foreground font-medium">
          {post.excerpt}
        </p>
        ...
        <div className="mt-10 rounded-2xl border border-primary/20 bg-primary/5 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Discover guided tours, local experiences, and hidden gems.
          </p>
          <Link
            href="/explore"
