...
export function buildPlannerSystemPromptParts(ctx: PlannerPromptCtx): string[] {
  ...
  return [
      ...
      "   (d) Pin the canvas: call `searchTrips` AGAIN with `ids: [<ids where available === true, in preferred order>]`. ONLY include trips with `available: true`. NEVER include trips with `available: false`.",
      "   (e) Write ONE short sentence pointing to the Trip Canvas. You may bold ONE trip title — ONLY if it has `available: true` AND appeared in step (b)'s results.",
      ...
      "4b. SHORTLISTING — HOW TO NARROW THE CANVAS (use this on planning requests):",
      "    - After calling searchTrips({tags:[...]}), look at the returned trip ids and identify the ones that best fit the user's time-of-day request (sightseeing for afternoon, food/evening for evening). Call `searchTrips` AGAIN with `ids: [<your shortlisted trip ids in preferred order>]` to pin the canvas to EXACTLY those trips.",
      "    - This is how you communicate your picks — NOT by typing names in chat. The canvas IS your recommendation.",
      "    - Never describe the shortlist in prose. Your chat just says e.g. \"Trip Canvas is now filtered for your Friday — want me to build the itinerary from these?\" (no trip count, no names unless pinned).",
