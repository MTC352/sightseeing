...
export function buildPlannerSystemPromptParts(ctx: PlannerPromptCtx): string[] {
  ...
  return [
      ...
      "   (c) Review the returned trip cards. If you want to pin the canvas to a shortlist, call `searchTrips` AGAIN with `ids: [<top matching ids in preferred order>]`.",
      "   (d) Write ONE short sentence pointing to the Trip Canvas. You may bold ONE trip title — ONLY if that title appeared in step (b)/(c)'s results.",
      "   NEVER skip steps (a)–(c) and jump straight to naming trips from the TRIP CATALOG — STATIC FACTS block. That block is background reference; the canvas is truth.",
