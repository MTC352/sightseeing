...
export function buildPlannerSystemPromptParts(ctx: PlannerPromptCtx): string[] {
  ...
  return [
      ...
      "12c. TRIP TIME-PREFERENCE REBUILD (CRITICAL — violations leave the canvas stale): When the visitor asks to schedule a specific trip at a specific time-of-day OR requests it be placed earlier/later in the day (examples: 'I want the Food Tour in the morning', 'put the Wine Tasting first', 'schedule the City Tour for afternoon', 'move the e-bike tour to 14:00', 'can we do the food tour earlier', 'morning food tour please') AND MY TRIP LIST has 1 or more trips — call buildItinerary IMMEDIATELY to rebuild the plan with that trip placed at its preferred time. NEVER respond with only a text description of the new schedule ('Your plan with X in the morning is on the Trip Canvas…') without also calling buildItinerary — the canvas only updates when the tool is called. Rules: (1) If the trip is already in MY TRIP LIST: call buildItinerary directly, placing the requested trip first (or at the requested time) in the steps array. (2) If the trip has a single available slot that matches the requested time-of-day, note this is a CONSTRAINT (e.g. '10:45 is the only available time — it counts as morning'). (3) Reply with ONE short neutral line only: 'Rebuilding your day with the Food Tour first — checking live slots now.' Then let the inline card show the result. NEVER pre-describe the finished schedule in the same turn as a buildItinerary call (see rule 12a).",
