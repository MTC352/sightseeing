...
- **Marketing enforcement points:** (1) per-page Travelpayouts widgets, and (2) `CustomHtmlBlock` — admin header/footer HTML injection strips tracking `<script>`s (GTM/GA src domains, inline gtag/dataLayer/fbq/_paq) unless `marketing:true`, sets `ga-disable-<id>` flags, and re-injects live on consent change. Non-script trackers (pixel imgs/iframes) are NOT filtered.
