...
- **Marketing enforcement points:** (1) per-page Travelpayouts widgets, and (2) `CustomHtmlBlock` — admin header/footer injection uses BLOCK-LEVEL gating: admin gated blocks (header "Google Analytics / Tracking", footer "Analytics & Tracking") are saved wrapped in `<!--consent:marketing-->…<!--/consent:marketing-->` markers; everything inside (scripts, noscript, iframes) is withheld until `marketing:true` and injected live on consent change. Content outside markers (Head Scripts/Meta Tags, Footer Widget, chat) ALWAYS loads. Legacy fallback: html with NO markers still gets pattern-based tracking-script stripping (GTM/GA src, inline gtag/dataLayer/fbq/_paq). `ga-disable-<id>` flags set when rejected. Admin page round-trips blocks via `splitMergedHtml` (label comments).
- **Real marketing enforcement for widgets is in the per-page Travelpayouts widgets**, NOT the banner. `flight-search-widget.tsx` / `train-search-widget.tsx` gate their `<script>` injection on `useConsent().marketing` and render `ConsentNotice` when blocked. The banner's `<TravelpayoutsAllowed/>` only sets a `window.__tpConsentGiven` marker that nothing reads — do not mistake it for the gate.
