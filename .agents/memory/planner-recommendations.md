...
- Date is a FILTER (not visual-grouping-only): when a date is selected the canvas shows ONLY trips bookable on that date (the on-date group). **Other-date trips are OPT-IN, not shown by default.** The render is a mutually-exclusive TWO-MODE view gated on `showOtherDates` (frozen-during-streaming as `canvasShowOtherDates`): default ON-DATE view; the OTHER-DATES view shows only the others group + a "Back to <date>" button. `showOtherDates` resets to false on date-change and on new `aiTrips`. **The reveal trigger is HIDDEN by default and has TWO opt-in paths that must stay in lockstep — both flip `setShowOtherDates(true)`:** (1) an inline TEXT-LINK (`data-testid="planner-see-other-dates"`, styled link not a big button) shown ONLY inside the zero-availability note when `onDate.length===0 && !availLoading && others.length>0`; and (2) the `showOtherDates` CLIENT tool (no server execute; defined+registered in route.ts, handled in page.tsx `onToolCall`) the AI calls ONLY when the visitor explicitly asks to see other dates (system rule 6d). Do NOT re-introduce an always-visible "See N trips on other dates" button. **INVARIANT — other-date trips must stay out of the AI's awareness until the visitor opts in:** `displayedTitles` is built from `visibleCanvasTrips` (which already mirrors the single visible mode), NOT from both groups. `otherDatesCount`/`otherDateSamples` remain a SEPARATE "you could offer these" hint so the AI can ASK (system-prompt "ASK BEFORE SHOWING OTHER DATES" + `showingOtherDates` mode), but those trips are never presented as "displayed". **Why (reversed prior direction):** a prior task forced BOTH groups (and AI awareness of both) to always render; the user explicitly wants other dates hidden from canvas AND AI until requested. Do NOT re-introduce always-show-both / make-AI-aware-of-both. **Why (weekend):** users asking "this weekend trips" saw all 18 catalog trips; they want only the available ones + the count. NOTE: "this weekend" resolves to a SINGLE Saturday startDate (merged date-resolution), so Sunday-only trips are excluded by design — making weekend a true Sat+Sun union would need a date-range model (prefs is single startDate).
